import argparse
from utility import GpsExtractor, Unspooler, prune_dataset, Visualizer, PerfWrapper, GPSWrapper, PerformanceVisualizer
...
def parse_args():
    ...
    # GPS Plotter
    gpsplot_parser = subparsers.add_parser("gpsplot")
    gpsplot_parser.add_argument("-v", "--videos", dest="video_fns", type=str, nargs='+', required=True, help="Videos to process. Tested on a GoPro Black 8")
    gpsplot_parser.add_argument("-o", "--out", dest="output", type=str, required=True, help="Output file.")
    
    # Local Performance
    # This way to complex to be used easily by a third user. However, I'll put it here for a later user.
    perfplot_parser = subparsers.add_parser("perfplot")
    # performance file is generated by the VPR_evaluation_framwork ExtendedResultsCSV
    perfplot_parser.add_argument("-p", "--perffile", dest="perffile", type=str, required=True, help="This file is generated by ResultsCSVExtended.py in VPR_Evlaution_Framwork. See main_evaluate_new.py for an example." )
    perfplot_parser.add_argument("-g", "--gpsfile", dest="gpsfile", type=str, required=True, help="This file is generated running the command gps with this main.py script." )
    perfplot_parser.add_argument("-m", "--metric", dest="metric", type=str, required=True, help="Metric to plot: AP,AUC,AP,F1." )
    perfplot_parser.add_argument("-o", "--output", dest="output", type=str, required=True, help="Output file." )
    # filters
    perfplot_parser.add_argument("-v", "--vpr", dest="vpr", type=str, required=True, help="VPR to visualize." )
    perfplot_parser.add_argument("-d", "--dataset", dest="ds", type=str, required=True, help="Dataset to visualize" )
    perfplot_parser.add_argument("-t", "--tolerance", dest="tolerance", type=str, required=True, help="tolerance to visualize (in meters)" )
...
# TEST
# gpsplot -v /home/main/Documents/MotionBlur/LUZZARA-04/raw_videos/loop_01.MP4 /home/main/Documents/MotionBlur/LUZZARA-04/raw_videos/loop_02.MP4 -o /home/main/Documents/MotionBlur/LUZZARA-04/raw_videos/plots/main_test.html
def command_gps_plot(args):
    visualizer = Visualizer()
    
    gps=list()
    for f in args.video_fns:
        gps.append(GpsExtractor(f, fill_the_gaps=False))
    for idx, l in enumerate(args.video_fns):
        name = os.path.basename(l)
        visualizer.append(name, gps[idx])
    visualizer.draw_map(args.output)    
    
# TEST
# perfplot -p /home/main/Documents/MotionBlur/results/result_files/by_place_results.txt -v AlexNet -d LUZZARA-04-02_to_01-L001 -t 10.0 -g /home/main/Documents/MotionBlur/LUZZARA-04/gps/LUZZARA-04-01.txt -m EP -o /home/main/Documents/MotionBlur/LUZZARA-04/raw_videos/plots/perf_test.html
def command_perf_plot(args):
    perf = PerfWrapper(args.perffile, args.metric)
    gps = GPSWrapper(args.gpsfile)
    t = str(float(args.tolerance))

    pv = PerformanceVisualizer(gps, perf, vpr=args.vpr, dataset=args.ds, tolerance = t)
    pv.draw_map(args.output)
...
COMMANDS = {
    ...
    "gpsplot" : command_gps_plot,
    "perfplot" : command_perf_plot,
}
